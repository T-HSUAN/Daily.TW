        ...
        <div class="ticket_list" v-if="ticketDisplay.length > 0">
            <div class="ticket_card" v-for="(item, index) in ticketDisplay" :key="item.id">
                ...
                <router-link :to="'/ticket/' + item.id" title="點擊查看票券詳情">
                    <Ticket :ticketPhoto="require(`@/assets/placeImg/${item.img}`)" :ticketTitle="item.Name"
                        :ticketLocation="item.location" :ticketTags="item.ticket_tag" :originalPrice="item.price_adult"
                        :FinalPrice="item.price_adult" :discountTag="item.discount" />
                </router-link>
                <div class="add_cart">
            ...
            <div class="item">
                ...
                <div class="details" v-for="(item, index) in cartItems" :key="item.id">
                    ...
                    <div class="ticket_ex">
                        <label for="count_ex">
                            <p class="name">優待票&nbsp;</p>
                            <p class="price">(NT$ {{ item.price_ex }} /張)</p>
                        </label>
