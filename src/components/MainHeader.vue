<template>
    ...
    <aside v-if="viewportWidth <= 768" :class="{ 'show-sidebar': showSidebar }">
        <img class="ham_close" :src="require('@/assets/img/layout/hamburger_close.svg')" alt="hamburger" @click="toggleSidebar" />
      <nav>
        <router-link v-for="item in menu" :key="item.id" :to="'/' + item.link" @click="flagDisplay(item.id)">
          <img :src="require(`@/assets/img/layout/${item.img}.svg`)" alt="icon" :style="{ display: item.display }" />
          &nbsp;{{ item.name }}
        </router-link>
      </nav>
